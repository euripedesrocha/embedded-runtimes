...
package Interfaces.LPC43xxC.EVENTROUTER is
   pragma Preelaborate;
   ...
   type HILO_Register is record
      --  Level detect mode for WAKEUP0 event.
      WAKEUP0_L  : ENUM := Interfaces.LPC43xxC.EVENTROUTER.Detect_Low_Level;
      --  Level detect mode for WAKEUP1 event. The corresponding bit in the
      --  EDGE register must be 0.
      WAKEUP1_L  : ENUM := Interfaces.LPC43xxC.EVENTROUTER.Detect_Low_Level;
      --  Level detect mode for WAKEUP2 event.
      WAKEUP2_L  : ENUM := Interfaces.LPC43xxC.EVENTROUTER.Detect_Low_Level;
      --  Level detect mode for WAKEUP3 event.
      WAKEUP3_L  : ENUM := Interfaces.LPC43xxC.EVENTROUTER.Detect_Low_Level;
      --  Level detect mode for alarm timer event.
      ATIMER_L   : ENUM := Interfaces.LPC43xxC.EVENTROUTER.Detect_Low_Level;
      --  Level detect mode for RTC event.
      RTC_L      : ENUM := Interfaces.LPC43xxC.EVENTROUTER.Detect_Low_Level;
      --  Level detect mode for BOD event.
      BOD_L      : ENUM := Interfaces.LPC43xxC.EVENTROUTER.Detect_Low_Level;
      --  Level detect mode for WWDT event.
      WWDT_L     : ENUM := Interfaces.LPC43xxC.EVENTROUTER.Detect_Low_Level;
      --  Level detect mode for Ethernet event
      ETH_L      : ENUM := Interfaces.LPC43xxC.EVENTROUTER.Detect_Low_Level;
      --  Level detect mode for USB0 event
      USB0_L     : ENUM := Interfaces.LPC43xxC.EVENTROUTER.Detect_Low_Level;
      --  Level detect mode for USB1 event
      USB1_L     : ENUM := Interfaces.LPC43xxC.EVENTROUTER.Detect_Low_Level;
      --  Level detect mode for SD/MMC event
      SDMMC_L    : ENUM := Interfaces.LPC43xxC.EVENTROUTER.Detect_Low_Level;
      --  Level detect mode for C_CAN event.
      CAN_L      : ENUM := Interfaces.LPC43xxC.EVENTROUTER.Detect_Low_Level;
      --  Level detect mode for combined timer output 2 event.
      TIM2_L     : ENUM := Interfaces.LPC43xxC.EVENTROUTER.Detect_Low_Level;
      --  Level detect mode for combined timer output 6 event.
      TIM6_L     : ENUM := Interfaces.LPC43xxC.EVENTROUTER.Detect_Low_Level;
      --  Level detect mode for QEI event.
      QEI_L      : ENUM := Interfaces.LPC43xxC.EVENTROUTER.Detect_Low_Level;
      --  Level detect mode for combined timer output 14 event.
      TIM14_L    : ENUM := Interfaces.LPC43xxC.EVENTROUTER.Detect_Low_Level;
      --  Reserved.
      RESERVED   : HILO_RESERVED_Field := 16#0#;
      --  Level detect mode for Reset
      RESET_L    : ENUM_1 :=
                    Interfaces.LPC43xxC.EVENTROUTER.Detect_Low_Level_If;
   ...
   type EDGE_Register is record
      --  Edge detect mode for WAKEUP0 event. The corresponding bit in the EDGE
      --  register must be 0.
      WAKEUP0_E  : ENUM_2 := Interfaces.LPC43xxC.EVENTROUTER.Level_Detect;
      --  Edge/level detect mode for WAKEUP1 event. The corresponding bit in
      --  the EDGE register must be 0.
      WAKEUP1_E  : ENUM_2 := Interfaces.LPC43xxC.EVENTROUTER.Level_Detect;
      --  Edge/level detect mode for WAKEUP2 event. The corresponding bit in
      --  the EDGE register must be 0.
      WAKEUP2_E  : ENUM_2 := Interfaces.LPC43xxC.EVENTROUTER.Level_Detect;
      --  Edge/level detect mode for WAKEUP3 event. The corresponding bit in
      --  the EDGE register must be 0.
      WAKEUP3_E  : ENUM_2 := Interfaces.LPC43xxC.EVENTROUTER.Level_Detect;
      --  Edge/level detect mode for alarm timer event. The corresponding bit
      --  in the EDGE register must be 0.
      ATIMER_E   : ENUM_3 := Interfaces.LPC43xxC.EVENTROUTER.Level_Detect;
      --  Edge/level detect mode for RTC event. The corresponding bit in the
      --  EDGE register must be 0.
      RTC_E      : ENUM_4 := Interfaces.LPC43xxC.EVENTROUTER.Level_Detect;
      --  Edge/level detect mode for BOD event. The corresponding bit in the
      --  EDGE register must be 0.
      BOD_E      : ENUM_5 := Interfaces.LPC43xxC.EVENTROUTER.Level_Detect;
      --  Edge/level detect mode for WWDTD event. The corresponding bit in the
      --  EDGE register must be 0.
      WWDT_E     : ENUM_6 := Interfaces.LPC43xxC.EVENTROUTER.Level_Detect;
      --  Edge/level detect mode for ethernet event. The corresponding bit in
      --  the EDGE register must be 0.
      ETH_E      : ENUM_7 := Interfaces.LPC43xxC.EVENTROUTER.Level_Detect;
      --  Edge/level detect mode for USB0 event. The corresponding bit in the
      --  EDGE register must be 0.
      USB0_E     : ENUM_8 := Interfaces.LPC43xxC.EVENTROUTER.Level_Detect;
      --  Edge/level detect mode for USB1 event. The corresponding bit in the
      --  EDGE register must be 0.
      USB1_E     : ENUM_8 := Interfaces.LPC43xxC.EVENTROUTER.Level_Detect;
      --  Edge/level detect mode for SD/MMC event.The corresponding bit in the
      --  EDGE register must be 0.
      SDMMC_E    : ENUM_9 := Interfaces.LPC43xxC.EVENTROUTER.Level_Detect;
      --  Edge/level detect mode for C_CAN event. The corresponding bit in the
      --  EDGE register must be 0.
      CAN_E      : ENUM_10 := Interfaces.LPC43xxC.EVENTROUTER.Level_Detect;
      --  Edge/level detect mode for combined timer output 2 event. The
      --  corresponding bit in the EDGE register must be 0.
      TIM2_E     : ENUM_11 := Interfaces.LPC43xxC.EVENTROUTER.Level_Detect;
      --  Edge/level detect mode for combined timer output 6 event. The
      --  corresponding bit in the EDGE register must be 0.
      TIM6_E     : ENUM_11 := Interfaces.LPC43xxC.EVENTROUTER.Level_Detect;
      --  Edge/level detect mode for QEI interrupt signal. The corresponding
      --  bit in the EDGE register must be 0.
      QEI_E      : ENUM_12 := Interfaces.LPC43xxC.EVENTROUTER.Level_Detect;
      --  Edge/level detect mode for combined timer output 14 event. The
      --  corresponding bit in the EDGE register must be 0.
      TIM14_E    : ENUM_11 := Interfaces.LPC43xxC.EVENTROUTER.Level_Detect;
      --  Reserved.
      RESERVED   : EDGE_RESERVED_Field := 16#0#;
      --  Edge/level detect mode for Reset. The corresponding bit in the EDGE
      --  register must be 0.
      RESET_E    : ENUM_4 := Interfaces.LPC43xxC.EVENTROUTER.Level_Detect;
      --  Edge detect of the BOD reset signal. The corresponding bit in the
      --  EDGE register must be 0.
      BODRESET_E : ENUM_4 := Interfaces.LPC43xxC.EVENTROUTER.Level_Detect;
      --  Edge detect of the deep power-down reset signal. The corresponding
      --  bit in the EDGE register must be 0.
      DPDRESET_E : ENUM_4 := Interfaces.LPC43xxC.EVENTROUTER.Level_Detect;
      --  Reserved.
...
end Interfaces.LPC43xxC.EVENTROUTER;
